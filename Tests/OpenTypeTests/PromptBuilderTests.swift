...
final class PromptBuilderTests: XCTestCase {
    ...
    func testSystemPromptIncludesChineseStyleScreenAndMemoryContext() {
        withCleanSettings {
            ...
            XCTAssertTrue(prompt.contains("输出：把 Utter 的 hotkey 文案改一下，不要影响菜单栏。"))
    ...
    func testSystemPromptIncludesEnglishStyleScreenAndMemoryContext() {
        withCleanSettings {
            ...
            XCTAssertTrue(prompt.contains("Use bullets for unordered collections and 1. 2. 3. only for explicit sequences or steps"))
            ...
            XCTAssertTrue(prompt.contains("Output: Update the Utter hotkey copy, and do not affect the menu bar."))
