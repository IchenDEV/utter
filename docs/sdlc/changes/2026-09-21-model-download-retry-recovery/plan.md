...
- [x] Real interrupted-download retry on a networked machine (network disruption limited to test download sessions without cutting host network): tested on both WhisperKit (`openai_whisper-tiny`) and HubApi (`mlx-community/Qwen2.5-0.5B-Instruct-4bit`):
      ...
      - Application resume path: `testApplicationModelCatalogResumePath` verified ModelCatalog download -> cancelDownload (paused state) -> awaited task settlement -> resume via downloadWhisper -> `.downloaded` (strictly within the scope of settled task completion; concurrent in-flight overlap without awaiting task completion at the UI facade layer is pending follow-up coverage).
